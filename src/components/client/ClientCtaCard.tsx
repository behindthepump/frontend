...
// The client dashboard's call-to-action: prompts logging.
export default function ClientCtaCard({ user, programStatus, onNavigate }: ClientCtaCardProps) {
  return (
    <div className="bg-[#111111] p-6 rounded-2xl text-white flex flex-col justify-between" id="dashboard-cta-card">
      <div className="space-y-3">
        ...
        <h3 className="text-lg font-extrabold tracking-tight">Daily Logging</h3>
        <p className="text-xs text-gray-400 leading-relaxed">
          {programStatus === "not_started"
            ...
            : `Every day you log builds your deficit and moves you closer to ${user.target_weight} kg. Small entries, big results.`}
        </p>
      </div>
      ...
      {programStatus !== "not_started" && (
        <div className="mt-6 space-y-2">
          ...
          >
            ...
            <span>{programStatus === "completed" ? "Review Your Days" : "Log Today's Calories"}</span>
          </button>
          <button
          ...
          >
            ...
            <span>{programStatus === "completed" ? "Review Your Workouts" : "Log This Week's Workouts"}</span>
          </button>
        </div>
      )}
    </div>
